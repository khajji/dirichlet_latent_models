...
"""
GIBBS SAMPLING IMPLEMENTATION FOR DIRICHLET LATENT MULTIMODAL REPRESENTATION (DLMR)
...
class Dlmr(object):
    ...
    """
    ----------------------------------PUBLIC METHODS------------------------------------
    fit(maxiter, maxiter_gibbs): infer the parameters of DLMR to fit the data: find optimal values for theta, phi, alpha, beta.
    ...
    def fit(self, maxiter, maxiter_gibbs):               
        ...
        for s in range(maxiter): #THIS IS THE BURNIN LOOP (i.e needed for the gibbs sampler to converge)
            ...
            if (alpha_mean_change <= Dlmr.hyper_parameter_dirichlet_converge_threshold and
                False not in [change <= Dlmr.hyper_parameter_dirichlet_converge_threshold for change in beta_mean_change]):
                ...
                break
        print("ENTRY IN THE PC MODE, #SAMPLES TO AVERAGE :" + str(Dlmr.hyper_parameter_number_of_samples_to_average))
        pc_samples = Dlmr.hyper_parameter_number_of_samples_to_average*Dlmr.hyper_parameter_interval_between_samples_to_average
        for s in range(pc_samples):  #ENTER IN THE POST CONVERGENCE (PC) MODE
            ...
            if s%Dlmr.hyper_parameter_interval_between_samples_to_average==0: #AFTER SOME ASSIGNMENTS, MAKE AN ESTIMATE OF THETA AND PHI
        ...
        self.theta /= Dlmr.hyper_parameter_number_of_samples_to_average         # AVERAGING GIBBS SAMPLES OF THETA
        for f in range(self.nb_features): self.phi[f]/= Dlmr.hyper_parameter_number_of_samples_to_average;  # AVERAGING GIBBS SAMPLES OF PHI
    ...
    def fit_newrecords(self, new_data, maxiter, maxiter_gibbs, chains_number):
        ...
        for chain in range(chains_number): #RUN A COMPLETE FITTING FROM SCRATCH
            ...
            for s in range(maxiter):    #THIS IS THE BURNIN LOOP (i.e needed for the gibbs sampler to converge)
                ...
                if (jll_change_rate <= Dlmr.hyper_parameter_llikelihood_converge_threshold):
                    #system converged --> leave the burnin loop
                    break
            print("ENTRY IN THE PC MODE, #SAMPLES TO AVERAGE :" + str(Dlmr.hyper_parameter_number_of_unseen_samples_to_average))                
            pc_samples = Dlmr.hyper_parameter_number_of_unseen_samples_to_average*Dlmr.hyper_parameter_interval_between_unseen_samples_to_average
            for s in range(pc_samples):      #ENTER IN THE POST CONVERGENCE (PC) MODE
                ...
                if s%Dlmr.hyper_parameter_interval_between_unseen_samples_to_average==0:
                    new_theta_averaged += new_theta
                    

            new_theta_averaged /= Dlmr.hyper_parameter_number_of_unseen_samples_to_average         # AVERAGING GIBBS SAMPLES OF THETA
    ...
    def _assign_hclasses(self, maxiter_gibbs):                         # DROW TOPIC SAMPLE FROM FULL-CONDITIONAL DISTRIBUTION
        for s in range(maxiter_gibbs):
            ...
            if((nbchanges/corpus_size) <= Dlmr.hyper_parameter_gibbs_converge_threshold): break; #LEAVE LOOP IF CONVERGENCE
...
if __name__ == "__main__":
    print("GIBBS SAMPLING FOR DIRICHLET LATENT MULTIMODAL REPRESENTATION (DLMR) INFERENCE.")
    data={0: {0:[0,0,1,4,5],
              1:[0,1,2,3]
              },
          1:  {0:[0,3,2],
    ...
    nb_iter = 100
    nb_gibbs_iter = 100
    dlmr = Dlmr(data, k, vocab_size)
    dlmr.fit(nb_iter, nb_gibbs_iter)
    print "-------------------FITTED PARAMS-------------------"
    print("|- THETA: "+str(dlmr.theta))
    print("|- PHI: "+str(dlmr.phi))
    ...
    chains_number = 5
    [new_theta, ll] = dlmr.fit_newrecords(new_data, nb_iter, nb_gibbs_iter, chains_number)
