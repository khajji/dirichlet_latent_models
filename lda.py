...
class Lda(object):
    ...
    def fit(self, maxiter, maxiter_gibbs):               
        ...
        for s in range(maxiter): #THIS IS THE BURNIN LOOP (i.e needed for the gibbs sampler to converge)
            ...
            beta_mean_change = np.mean(abs(self.beta - old_beta))

            jll = self._joint_ll(); print("SAMPLE #" + str(s)); print("|- Joint Likelihood:", jll)

            if (alpha_mean_change <= Lda.hyper_parameter_dirichlet_converge_threshold and
                False not in [change <= Lda.hyper_parameter_dirichlet_converge_threshold for change in beta_mean_change]):
